...
compileProgram :: Program -> Code
compileProgram program = concatMap compStm program

compile :: String -> Code
compile input = compileProgram (parse input)
...
main :: IO ()
main = do
    let input = "x := 5 ; if x == y then x := 1 ; else y := 2 ; while x <= y do x := x + 1 ;"
        compiledCode = compile input
    ...
    let (finalCode, finalStack, finalState) = run (compiledCode, initialStack, initialState)
